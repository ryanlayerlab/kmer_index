...
def extend_forward(T, read, read_i, T_i, max_mismatches):
    return None

def extend_backward(T, read, read_i, T_i, max_mismatches):
    return None

def extend(T, read, read_i, T_i, k, max_mismatches):
    f_read_i, f_T_i = extend_forward(T,
                                     read,
                                     read_i + k,
                                     T_i + k,
                                     max_mismatches)
    b_read_i, b_T_i = extend_backward(T,
                                      read,
                                      read_i,
                                      T_i,
                                      max_mismatches)

    return (b_T_i, f_T_i), (b_read_i, f_read_i)
